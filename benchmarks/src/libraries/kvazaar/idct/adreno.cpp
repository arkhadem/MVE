...
void idct_InitGPU(config_t *config) {
    FILE *fp;
    char *source_str;
    size_t source_size;
...
void idct_DestroyGPU(config_t *config) {
...
timing_t idct_adreno(config_t *config,
    ...
    CLOCK_START()
    // Create the input and output arrays in device memory for our calculation
    cl_mem d_in = clCreateBuffer(idct_context, CL_MEM_READ_ONLY | CL_MEM_ALLOC_HOST_PTR, inout_size, NULL, NULL);
    cl_mem d_out = clCreateBuffer(idct_context, CL_MEM_WRITE_ONLY | CL_MEM_ALLOC_HOST_PTR, inout_size, NULL, NULL);
    ...
    CLOCK_START()
    int16_t *h_in = (int16_t *)clEnqueueMapBuffer(idc_tqueue, d_in, CL_FALSE, CL_MAP_READ, 0, inout_size, 0, NULL, NULL, &err);
    int16_t *h_out = (int16_t *)clEnqueueMapBuffer(idc_tqueue, d_out, CL_FALSE, CL_MAP_WRITE, 0, inout_size, 0, NULL, NULL, &err);
    ...
    err |= clSetKernelArg(idct_kernel, 3, sizeof(cl_mem), &d_in);
    err |= clSetKernelArg(idct_kernel, 4, sizeof(cl_mem), &d_out);
