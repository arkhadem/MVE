...
void row_opaque_neon(int LANE_NUM,
                     config_t *config,
                     input_t *input,
                     output_t *output) {
    ...
    for (int out_y = 0; out_y < num_rows; out_y++) {
        ...
        uint8x8_t vopqDstA = vdup_n_u8(SkGetPackedA32(opaque_dst[out_y]));
        uint8x8_t vopqDstR = vdup_n_u8(SkGetPackedR32(opaque_dst[out_y]));
        uint8x8_t vopqDstG = vdup_n_u8(SkGetPackedG32(opaque_dst[out_y]));
        uint8x8_t vopqDstB = vdup_n_u8(SkGetPackedB32(opaque_dst[out_y]));
