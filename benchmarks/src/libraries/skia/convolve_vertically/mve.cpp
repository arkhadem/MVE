...
void convolve_vertically_mve(int LANE_NUM,
                             config_t *config,
                             input_t *input,
                             output_t *output) {
    ...
    uint8_t **out_row = (uint8_t **)convolve_vertically_output->out_row;
    ...
    // randomely across rows
    __vidx_var src_dst_stride = {2, 2, 0, 0};
    ...
    while (row < num_rows) {
        ...
        while (col < num_cols) {
            ...
            uint8_t **my_src_addr = src_data;

            __mdvdw acc_dw = _mve_set1_dw(0);

            for (int coeff_idx = 0; coeff_idx < filter_length; coeff_idx++) {
                __mdvb src_b = _mve_loadro_b((const __uint8_t **)my_src_addr, col, src_dst_stride);
            ...
            _mve_storero_b(out_row, col, acc_b, src_dst_stride);
            // free acc_b
            _mve_free_b();
