...
void gemm_neon(int LANE_NUM,
    ...
    while (m + 32 <= M) {
        for (int n = 0; n < N; n++) {
            ...
            const int32_t *input_addr = (const int32_t *)(in + m);
            const int32_t *weight_addr = (const int32_t *)(weights + n * K);
    ...
    for (int n = 0; n < N; n++) {
        for (int m_l = m; m_l < M; m_l++) {
            ...
            for (uint32_t k = 0; k < K; k++) {
                acc += (in[input_idx] * weights[weight_idx]);
                input_idx += M;
                weight_idx += 1;
            }
            acc = acc < max ? acc : max;
