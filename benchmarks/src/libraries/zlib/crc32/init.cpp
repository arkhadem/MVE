...
z_crc_t mve_crc_coeff[8192];
z_crc_t Coeff_64K_shift;

int crc32_init(size_t cache_size,
               int LANE_NUM,
               config_t *&config,
               input_t **&input,
               output_t **&output) {
    ...
    init_1D<crc32_config_t>(1, crc32_config);
    ...
    init_1D<crc32_input_t *>(count, crc32_input);
    init_1D<crc32_output_t *>(count, crc32_output);

    // initializing individual versions
    for (int i = 0; i < count; i++) {
        init_1D<crc32_input_t>(1, crc32_input[i]);
        init_1D<crc32_output_t>(1, crc32_output[i]);

        random_init_1D<unsigned char>(length, crc32_input[i]->buf);
        random_init_1D<z_crc_t>(1, crc32_output[i]->return_value);
    }

    for (int i = 0; i < 8192; i++) {
        // offline: calculating shift coeffs
        mve_crc_coeff[i] = x2nmodp(8 * (8191 - i), 3);
    }
    Coeff_64K_shift = x2nmodp(64 * 1024, 3);
