...
#define LOAD()                                                 \
    src_b = _mve_loadro_b(src_addr, offset, buff_load_stride); \
    offset += 8192;                                            \
    _mve_cmpeq_b(src_b, value_b);                              \
    /* free src_b */                                           \
    _mve_free_b();                                             \
...
void memchr_mve(int LANE_NUM,
    ...
    const __uint8_t *src = (const __uint8_t *)memchr_input->src;
    const __uint8_t **src_addr = (const __uint8_t **)memchr_input->src_addr;
    ...
    if (min_idx != 10000000)
        memchr_output->return_value = (char *)src + min_idx;
    else
        memchr_output->return_value = nullptr;
}