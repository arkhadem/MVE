...
int aes_init(size_t cache_size,
             int LANE_NUM,
             config_t *&config,
             input_t **&input,
             output_t **&output) {
    ...
    init_1D<aes_config_t>(1, aes_config);
    aes_config->num_blocks = num_blocks;
    KeyExpansion(aes_config->RoundKey);

    // in/output versions
    size_t input_size = num_blocks * 16 * sizeof(unsigned char);
    size_t output_size = 0;
    int count = cache_size / (input_size + output_size) + 1;

    // initializing in/output versions
    init_1D<aes_input_t *>(count, aes_input);
    init_1D<aes_output_t *>(count, aes_output);

    // initializing individual versions
    for (int i = 0; i < count; i++) {
        init_1D<aes_input_t>(1, aes_input[i]);
        init_1D<aes_output_t>(1, aes_output[i]);

        random_init_1D<unsigned char>(num_blocks * 16, aes_input[i]->state);
    }
