...
def get_libraries_kernels(scheme, isa, args):
...
def run_dynamorio(directory, schemes, isas, args):
...
def run_compiler(directory, schemes, isas, args):
	for scheme in schemes:
		for isa in isas:
...
def parse_simulation(directory, schemes, isas, args):
	for scheme in schemes:
		for isa in isas:
			output = f"{isa}_{scheme}.csv"
...
def main():
	...
	if action == "benchmark":
		run_dynamorio(directory, schemes, isas, args)
	elif action == "compile":
		run_compiler(directory, schemes, isas, args)
	elif action == "simulate":
		run_simulator(directory, schemes, isas, args)
	elif action == "parse":
		parse_simulation(directory, schemes, isas, args)
	else:
